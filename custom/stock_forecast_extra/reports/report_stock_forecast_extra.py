...
class ReportStockForecastExtra(models.Model):
    _name = 'report.stock.forecast.extra'
    _auto = False
    _description = 'Stock Forecast Report enhanced'
    ...
    def init(self):
        tools.drop_view_if_exists(self._cr, 'report_stock_forecast_extra')
        query = """CREATE or REPLACE VIEW report_stock_forecast_extra AS (
SELECT
    MIN(id) as id,
    ...
    (SELECT
		...
		LEFT JOIN
        product_template ON product_template.id = sq.product_id
        LEFT JOIN
        stock_location location_id ON sq.location_id = location_id.id
        WHERE
        location_id.usage = 'internal'
        GROUP BY date, sq.product_id, sq.company_id, product_template.categ_id
        UNION ALL
        SELECT
        MIN(-sm.id) as id,
        sm.product_id,
		...
		LEFT JOIN
        product_template ON product_template.id = sm.product_id
        LEFT JOIN
        ...
        GROUP BY sm.date_deadline,sm.product_id, sm.company_id, product_template.categ_id
        UNION ALL
        SELECT
            MIN(-sm.id) as id,
			sm.product_id,
...
GROUP BY MAIN.product_id,SUB.date, MAIN.date, MAIN.company_id, MAIN.categ_id
) AS FINAL
GROUP BY product_id,date,company_id, categ_id
)"""
